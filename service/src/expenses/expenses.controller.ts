...
export class ExpensesController {
  ...
  list(@Query() query: ListQuery, @CurrentUser() user?: JwtPayload) {
    return this.notionService.list('expenses', query, user?.id);
  }

  @Get(':id')
  detail(@Param('id') id: string, @CurrentUser() user?: JwtPayload) {
    return this.notionService.detail('expenses', id, user?.id);
  }

  @Post()
  @UseGuards(JwtAuthGuard)
  create(@Body() body: Record<string, unknown>, @CurrentUser() user: JwtPayload) {
    return this.notionService.create('expenses', body, user.id);
  }

  @Patch(':id')
  @UseGuards(JwtAuthGuard)
  update(@Param('id') id: string, @Body() body: Record<string, unknown>, @CurrentUser() user: JwtPayload) {
    return this.notionService.update('expenses', id, body, user.id);
  }

  @Post('bulk-create')
  @UseGuards(JwtAuthGuard)
  bulkCreate(@Body() body: { items: Record<string, unknown>[] }, @CurrentUser() user: JwtPayload) {
    return this.notionService.bulkCreate('expenses', body.items ?? [], user.id);
  }

  @Post('bulk-delete')
  @UseGuards(JwtAuthGuard)
  bulkDelete(@Body() body: { ids: string[] }, @CurrentUser() user: JwtPayload) {
    return this.notionService.bulkDelete('expenses', body.ids ?? [], user.id);
  }

  @Delete(':id')
  @UseGuards(JwtAuthGuard)
  delete(@Param('id') id: string, @CurrentUser() user: JwtPayload) {
    return this.notionService.delete('expenses', id, user.id);
  }
}
